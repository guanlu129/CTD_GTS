...
CNVfile = 'XXXX-XXX-XXXX.cnv'
dest = 'C:/XXXX/XXXX/XXXX/XXXX/'
...
#T0 = cnv.data['T0']
prDM = cnv.data['prDM']   #prDM = SBE911 - change code for SBE911
#p = cnv.data['p']
c0 = cnv.data['c0S/m']
#C0 = cnv.data['C0']
...
lat = profile.attributes['LATITUDE']              #------------!!! SH notes - this won't be recognized if there no 'N' after latitude in the cnv file!!!.
...
water_depth = cnv.header.split('**')[3].split('*')[0][8:-1]
#water_depth = water_depth.replace(' m', '')                # use this if the cnv file has ' m' after the depth val
water_depth = water_depth.strip()                           # use this if there is an extra space before the depth val
